local Library do
    ...
    function Library:AddDraggableButton(Text, Func, ExcludeScaling)
        ...
        local function UpdateSize()
            local textBounds = Button.Instance.TextBounds
            local width = textBounds.X + 24
            local height = textBounds.Y + 12
            Button.Instance.Size = UDim2.fromOffset(width, height)
        end
        ...
        Button.Instance:GetPropertyChangedSignal("Text"):Connect(UpdateSize)
        ...
        Button:Connect("InputBegan", function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                dragging = true
                dragStart = input.Position
                startPos = Button.Instance.Position
        ...
        Library:Connect(UserInputService.InputChanged, function(input)
            if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                local buttonSize = Button.Instance.AbsoluteSize
                newX = math.clamp(newX, 0, screenSize.X - buttonSize.X)
                newY = math.clamp(newY, 0, screenSize.Y - buttonSize.Y)
                Button.Instance.Position = UDim2.fromOffset(newX, newY)
            end
        end)
        
        Button:Connect("MouseButton1Click", function()
            if Func then Func(Table) end
        end)
        
        Table.Button = Button
        Table.SetText = function(self, newText)
            Button.Instance.Text = newText
        end
        
        table.insert(Library.DraggableButtons, Table)
        return Table
    end
    ...
    Library.Window = function(self, Data)
        ...
        local Window = {
            ...
            Draggable = true,
            MobileButtonsSide = Data.MobileButtonsSide or "Left"
        }
        ...
        local Items = { } do
            ...
            Items["MainFrame"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    if not Window.Draggable or Library.CantDragForced then return end
                    Dragging = true
            ...
            if IsMobile then
                ...
                if Window.MobileButtonsSide == "Right" then
                    toggleButton.Button.Instance.Position = UDim2.new(1, -6, 0, 6)
                    toggleButton.Button.Instance.AnchorPoint = Vector2.new(1, 0)
                    lockButton.Button.Instance.Position = UDim2.new(1, -6, 0, 46)
                    lockButton.Button.Instance.AnchorPoint = Vector2.new(1, 0)
                else
                    lockButton.Button.Instance.Position = UDim2.fromOffset(6, 46)
                end
            end
            ...
            function Window:SetTransparency()
                ...
                if IsMobile then
                    for _, btn in Library.DraggableButtons do
                        btn.Button.Instance.BackgroundTransparency = Library.Flags["BackgroundTransparency"]
                    end
                end
